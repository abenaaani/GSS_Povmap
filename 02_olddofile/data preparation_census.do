...
drop related0to17 related0to5 related_persons workers_in_family ///
...
save census123,replace
s
save census569,replace
s
save census47810,replace
...
use "$census\10%Livestock", clear
collapse (sum) numlivestock = LIVESTIC, by(HHID)
merge 1:m HHID using `census'
drop _me
order numlivestock, last
recode numlivestock (. = 0)
egen numlivestock_eamean = mean(numlivestock), by(region district ea)
egen numlivestock_distmean = mean(numlivestock), by(region district)
egen numlivestock_regionmean = mean(numlivestock), by(region)
*/
...
preserve
duplicates drop region district ea, force
keep region district ea
...
local vars hhsize head_male male head_age age depratio head_ghanaian ghanaian head_ethnicity1 - head_ethnicity10 ///
		...
		mobileown internetuse /*birth12mo surviverate */fixedphone pc aghouse conventional wall1 - wall3 floor1 - floor3 roof1 - roof4 tenure1 - tenure3 rooms bedrooms ///
...
local vars hhsize head_male male head_age age depratio head_ghanaian ghanaian head_ethnicity1 - head_ethnicity10 ///
		...
		noschooling head_schlvl1 - head_schlvl5 employed head_occ1 - head_occ11 head_empstatus1 - head_empstatus9 employee ///
		mobileown internetuse /*surviverate birth12mo*/ fixedphone pc aghouse conventional wall1 - wall3 floor1 - floor3 roof1 - roof4 tenure1 - tenure3 rooms bedrooms ///
		lighting1 - lighting4 water_drinking1 - water_drinking4 water_general1 - water_general3 fuel1 - fuel4 toilet1 - toilet5 solidwaste1 - solidwaste4 /*numlivestock*/
foreach x of varlist `vars' {
keep region district ea `x'
